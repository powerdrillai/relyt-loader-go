...
func CreateTestDataTale(db *sql.DB) error {
	...
	// CREATE TABLE relyt_bulk_insert_benchmark (id bigint NOT NULL PRIMARY KEY, ext TEXT, vector vecf16(3) NOT NULL);
	log.Println("Creating relyt_bulk_insert_benchmark in PostgreSQL...")
	query := `
	CREATE TABLE IF NOT EXISTS relyt_bulk_insert_benchmark (
	...
	_, err := db.Exec(query)
	if err != nil {
		return fmt.Errorf("failed to create relyt_bulk_insert_benchmark: %w", err)
	}
	log.Println("relyt_bulk_insert_benchmark created successfully.")
	return nil
}

func TruncateTestDataTable(db *sql.DB) error {
	// This function is a placeholder for truncating the relyt_bulk_insert_benchmark in PostgreSQL.
	// You can implement the logic to truncate the table here.
	log.Println("Truncating relyt_bulk_insert_benchmark in PostgreSQL...")
	query := `TRUNCATE TABLE relyt_bulk_insert_benchmark;`
	_, err := db.Exec(query)
	if err != nil {
		return fmt.Errorf("failed to truncate relyt_bulk_insert_benchmark: %w", err)
	}
	log.Println("relyt_bulk_insert_benchmark truncated successfully.")
	return nil
}

func GetCountFromTestDataTable(db *sql.DB) (int, error) {
	// This function retrieves the count of records in the relyt_bulk_insert_benchmark.
	log.Println("Counting records in relyt_bulk_insert_benchmark...")
	query := `SELECT COUNT(*) FROM relyt_bulk_insert_benchmark;`
	var count int
	err := db.QueryRow(query).Scan(&count)
	if err != nil {
		return 0, fmt.Errorf("failed to count records: %w", err)
	}
	log.Printf("Counted %d records in relyt_bulk_insert_benchmark.", count)
	return count, nil
}
